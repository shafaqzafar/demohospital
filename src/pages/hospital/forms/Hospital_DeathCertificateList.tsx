...
export default function Hospital_DeathCertificateList(){
  ...
  async function load(){
    setLoading(true)
    try {
      const res: any = await hospitalApi.listIpdDeathCertificates({ q, page, limit }).catch(()=>null)
  ...
  async function onPrint(encounterId: string){
    try {
      const html = await coreApi(`/hospital/ipd/admissions/${encodeURIComponent(encounterId)}/death-certificate/print`) as any
  ...
  async function onDelete(encounterId: string){
    ...
    try { await hospitalApi.deleteIpdDeathCertificate(encounterId) } catch {}
    load()
  }

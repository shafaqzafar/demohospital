...
export default function Lab_Header({ onToggleSidebar, onToggleTheme, theme }: { onToggleSidebar?: ()=>void; onToggleTheme?: ()=>void; theme?: 'light'|'dark' }) {
  ...
  return (
    <header className="sticky top-0 z-10 h-16 w-full border-b border-slate-200 bg-white/90 backdrop-blur dark:border-slate-700 dark:bg-slate-900/80">
      <div className="mx-auto flex h-full max-w-7xl items-center gap-3 px-4 sm:px-6">
        {onToggleSidebar ? (
          <button type="button" onClick={onToggleSidebar} className="inline-flex items-center justify-center rounded-md border border-slate-200 p-2 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
        ...
        <Link to="/lab" className="flex items-center gap-2">
          ...
          <div className="font-semibold text-slate-900 dark:text-slate-100">Lab</div>
        ...
        <div className="ml-auto flex items-center gap-3 text-sm">
          ...
          <button type="button" aria-pressed={dark} onClick={onToggleTheme} className="hidden rounded-md border border-slate-200 px-3 py-1.5 text-slate-700 hover:bg-slate-50 sm:flex items-center gap-2 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
          ...
          <div className="rounded-md border border-slate-200 px-3 py-1.5 text-slate-700 dark:border-slate-700 dark:text-slate-200">admin</div>
        </div>
      </div>
    </header>
  )

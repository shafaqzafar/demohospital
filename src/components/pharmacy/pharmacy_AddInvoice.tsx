...
export default function Pharmacy_AddInvoice({ open, onClose }: Props) {
      ...
      <div className="w-full max-w-6xl max-h-[90vh] flex flex-col rounded-xl bg-white p-0 shadow-2xl ring-1 ring-black/5">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 px-6 py-5 text-sm">
              ...
              {items.map((row) => (
                <div key={row.id} className="grid gap-4 md:grid-cols-3">
                  ...
                  <div>
                    ...
                    <input disabled value={((row.buyPerPack||0) * (row.packs||0)).toFixed(2)} className="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2" />
                  </div>
                  <div>
          ...
          <section className="rounded-lg border border-slate-200">
            ...
            <div className="space-y-3 p-4 text-xs text-slate-600">Add invoice-level taxes such as Advance WHT, Delivery, etc.</div>
            {taxes.map(t => (
